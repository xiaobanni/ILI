...
PIXEL_MODE = False
# first render at multiple of Pixel Obs resolution, then downscale. Looks better.
PIXEL_SCALE = 4
...
class RelativeState:
    ...
    def getObservation(self):
        ...
        # scale inputs to be in the order of magnitude of 10 for neural network.
        scaleFactor = 10.0
...
class Agent:
    ...
    def display(self, canvas, bx, by):
        ...
        canvas = circle(canvas, toX(x+(0.6)*r*c), toY(y+(0.6)
                        * r*s), toP(r)*0.3, color=(255, 255, 255))
        canvas = circle(canvas, toX(x+(0.6)*r*c+eyeX*0.15*r),
                        toY(y+(0.6)*r*s+eyeY*0.15*r), toP(r)*0.1, color=(0, 0, 0))

        # draw coins (lives) left
        for i in range(1, self.life):
            canvas = circle(canvas, toX(self.dir*(REF_W/2+0.5-i*2.)),
                            WINDOW_HEIGHT-toY(1.5), toP(0.5), color=COIN_COLOR)
...
class Game:
    ...
    def reset(self):
        ...
        self.fence = Wall(0, 0.75 + REF_WALL_HEIGHT/2,
                          REF_WALL_WIDTH, (REF_WALL_HEIGHT-1.5), c=FENCE_COLOR)
...
class SlimeVolleyEnv(gym.Env):
    ...
    def step(self, action, otherAction=None):
        ...
        # value in value_state has already been divided by factor 10 in getObservation function
        value_state = self.get_processed_value_state()

        if self.survival_bonus:
            return {"state":value_state,"pixels":obs}, reward+0.01, done, info
        return {"state":value_state,"pixels":obs}, reward, done, info
    ...
    def reset(self):
        ...
        return {"state": self.get_processed_value_state(), "pixels": self.getObs()}
    ...
    def checkViewer(self):
        ...
        if self.viewer is None:
            ...
            self.viewer = rendering.SimpleImageViewer(
                maxwidth=2160)  # macbook pro resolution
    ...
    def get_processed_value_state(self):
        value_state = self.game.agent_right.getObservation()
        x_dis = value_state[0] - value_state[4]
        y_dis = value_state[1] - value_state[5]
        vx_dis = value_state[2] - value_state[6]
        vy_dis = value_state[3] - value_state[7]

        return np.array([x_dis, y_dis, vx_dis, vy_dis])
...
class SlimeVolleySymbolicAtariEnv(SlimeVolleyEnv):
    from_pixels = True
    atari_mode = True
    ...
    def reset(self):
        self.init_game_state()
        return self.get_processed_value_state()
    ...
    def step(self, action, otherAction=None):
        ...
        if self.survival_bonus:
            return value_state, reward + 0.01, done, info
        return value_state, reward, done, info
...
def render_atari(obs):
    ...
    latest = cv2.resize(latest, (84 * 8, 84 * 4),
    ...
    tempObs = cv2.resize(tempObs, (84 * 8, 84 * 2),
                         interpolation=cv2.INTER_NEAREST)
...
register(
    id='SlimeVolleySymbolic-v0',
    entry_point='slimevolleygym.slimevolley:SlimeVolleySymbolicAtariEnv'
)
