...
class RoomObjects(MiniWorldEnv, utils.EzPickle):
    ...
    def _gen_world(self):
        ...
        self.place_entity(
            Box(color=colorlist[self.np_random.choice(
                len(colorlist))], size=0.9)
        )
        self.place_entity(
            Ball(color=colorlist[self.np_random.choice(
                len(colorlist))], size=0.9)
        )
        self.place_entity(
            Key(color=colorlist[self.np_random.choice(len(colorlist))]))
