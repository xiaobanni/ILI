...
class BigKey(Key):
    ...
    def __init__(self, color, size=0.6):
        ...
        MeshEnt.__init__(
            self, mesh_name=f"key_{color}", height=size, static=False)
...
class Sign(MiniWorldEnv, utils.EzPickle):
    ...
    def __init__(self, size=10, max_episode_steps=20, color_index=0, goal=0, **kwargs):
        if color_index not in [0, 1, 2]:
            raise ValueError(
                "Only supported values for color_index are 0, 1, 2.")
        ...
        self.observation_space = Dict(
            obs=self.observation_space, goal=Discrete(2))
    ...
    def _gen_world(self):
        ...
        self.connect_rooms(top_room, left_room, min_x=0,
                           max_x=self._size * 3 / 5)
        self.connect_rooms(
            left_room,
            right_room,
